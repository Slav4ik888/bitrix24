...
class CApplication
{
		...
		switch ($operation){
			...
			case 'uninstall': 
				...
				try {
					...
					$res = $db->query('DELETE FROM `b24_ratings` '.
                        'WHERE ID in ('.
                            'SELECT * FROM ('.
                            '    SELECT rating.ID '.
                            '    FROM `b24_users` as users '.
                            '    JOIN `b24_ratings` as rating ON (rating.ID_USER = users.ID)'.
                            '    WHERE users.`PORTAL` = ?s'.
                            ') as IDS'.
                        ')', $params['domain']);

                    $res = $db->query('DELETE FROM `b24_users` WHERE `PORTAL` = ?s', $params['domain']);
                    $res = $db->query('DELETE FROM `b24_portal_reg` WHERE `PORTAL` = ?s', $params['domain']);
	...
	public function start () {
		...
		if ($_REQUEST['operation'] != 'uninstall')
